...
SamlIdp.configure do |config|
  ...
  if Rails.env.production? # TODO: use valid certificate for production
  ...
  config.name_id.formats = 
    {
      "1.1" => {
        email_address: -> (principal) { principal.identity_key },
      },
      "2.0" => {
        transient: -> (principal) { principal.identity_key },
  ...
  config.organization_name = "Our Super Special Solid Organization"
  config.organization_url = "#{base}"
  config.base_saml_location = "#{base}saml/" # represents: Issuer
  config.entity_id = "#{base}saml/metadata/"
  ...
  config.attribute_service_location = "#{base}saml/metadata/"
  ...
  service_providers = {
    "http://localhost:3000/backend_tests/authentication/saml/metadata" => {
      fingerprint: "9E:65:2E:03:06:8D:80:F2:86:C7:6C:77:A1:D9:14:97:0A:4D:F4:4D",
      metadata_url: "http://localhost:3000/backend_tests/authentication/saml/metadata",
